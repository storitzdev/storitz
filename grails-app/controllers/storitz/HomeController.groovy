    ...
    for (site in sites) {
      ...
      if (bestUnit) {
        if (site.featuredOffers().size() == 0) {
          ...
          siteMoveInPrice[site.id] = [cost:totals['moveInTotal'], promo:null, promoName:null, monthly:bestUnit?.pushRate, paidThruDate:totals['paidThruDate'], sizeDescription: bestUnit?.displaySize, unitType:bestUnit?.unitType?.display]
        } else {
          for (promo in site.featuredOffers()) {
            ...
            if (!siteMoveInPrice[site.id] || siteMoveInPrice[site.id].cost > totals['cost']) {
              siteMoveInPrice[site.id] = [cost:totals['moveInTotal'], promo:promo.id, promoName:promo.promoName, monthly:bestUnit?.pushRate, paidThruDate:totals['paidThruDate'], sizeDescription: bestUnit?.displaySize, unitType:bestUnit?.unitType?.display]
            }
          }
        }
      }

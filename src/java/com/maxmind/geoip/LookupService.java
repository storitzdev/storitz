...
public class LookupService {
     ...
     * @throws java.io.IOException if file cannot be opened
     */
	...
	int [] x = new int[2];
        ...
        for (int depth = 31; depth >= 0; depth--) {
            if ((dboptions & GEOIP_MEMORY_CACHE) == 1) {
		//read from memory
                System.arraycopy(dbbuffer, (2 * recordLength * offset), buf, 0, 2 * MAX_RECORD_LENGTH);
            } else if ((dboptions & GEOIP_INDEX_CACHE) != 0) {
                //read from index cache
                System.arraycopy(index_cache, (2 * recordLength * offset), buf, 0, 2 * MAX_RECORD_LENGTH);
            } else {
		//read from disk 
		try {

...
function createTableRow(s) {
  ...
  if (s.logo.toString().length > 0) {
    logoCol = $('<div>').append($('<div>', { 'class': 'left' } ).css('width','120px').append($('<img>', { 'class':'stLogo', src: s.logo, width:100, height:40, border:0, alt: s.title + ' Logo'})))
                        .append($('<div>', { 'class': 'stCompany' }).css('visibility','hidden').text(s.operatorName));
  } else {
  ...
  if (s.promoId) {
    ...
    offersCol
      .append($('<div>', {'class':'left'}).css({'width':'22px', 'height':'100%'})
      .append($('<div>', {'class':'stSpecialOffers'}).css({'visibility':'hidden'}).text('ZZZ')))
...
function setupTable() {
    ...
    $.fn.dataTableExt.afnSortData['specials'] = function  (oSettings, iColumn, vColumn) {
        var aData = [];
        $('td:eq(' + vColumn + ') div.stSpecialOffers', oSettings.oApi._fnGetTrNodes(oSettings)).each(function () {
            aData.push($(this).text());
        });
        return aData;
    }
    $.fn.dataTableExt.afnSortData['company'] = function  (oSettings, iColumn, vColumn) {
        var aData = [];
        $('td:eq(' + vColumn + ') div.stCompany', oSettings.oApi._fnGetTrNodes(oSettings)).each(function () {
            aData.push($(this).text());
        });
        return aData;
    }
...
function buildState() {
    ...
    params.searchType = $('input[name="searchType"]').val();
...
function setupRebateLink() {
    ...
    $('#btn-rent').click(function(event) {
        ...
        $('#rebate').dialog({position:[event.pageX-100,event.pageY]}).dialog('open');
    });
}
